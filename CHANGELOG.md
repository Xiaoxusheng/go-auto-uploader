...
这是一份为你量身定制的 **v1.7.0** 专属迭代更新日志。

我已经将刚才关于 **WebSocket 彻底接管高频轮询**、**日志终端物理级还原** 以及 **Cookie 安全解析引擎** 的重大架构与交互升级全部浓缩其中。你可以直接将其追加到你的 `README.md` 中：
...
* 后端独创了 **“无损 INI 精准覆写算法”**。告别粗暴的纯文本整体覆盖，后端现支持逐行扫描定位、精确切片替换对应的 `_cookie` 键值对（若无节点则智能追加），绝对保障引擎主配置 (`config.ini`) 中其他参数的安全与完整。
